...
class AdminSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = NewUser
        fields=['id','email','user_name','responsable','password','commune','first_name','adresse','about_me','is_user','owner','is_active','is_staff','last_login','profile_image']
        extra_kwargs ={
            'password':{'write_only':True}
        }
...
class SuperAdminSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = NewUser
        fields=['id','email','user_name','responsable','commune','first_name','adresse','about_me','is_active','is_staff','is_superuser','owner','last_login','updated_at','profile_image']
        extra_kwargs ={
            'password':{'write_only':True}
        }
...
class GeneraleSerialiser(serializers. HyperlinkedModelSerializer):
    class Meta:
        model = NewUser
        fields=['id','start_date','responsable','email','user_name','commune','first_name','adresse','about_me','is_user','is_superuser','is_agent','is_active','is_staff','profile_image','create','last_login','updated_at']
        extra_kwargs ={
            'password':{'write_only':True}
        }
